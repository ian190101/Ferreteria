...
class StoreProductRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'barcode' => [app(ProductWorkflowPolicy::class)->barcodeRequired() ? 'required' : 'nullable', 'string', 'max:80', 'unique:products,barcode'],
    ...
    private function validateFlexibleCatalog(Validator $validator): void
    {
        ...
        if ($variantSkus->isNotEmpty() && ProductVariant::query()->whereIn('sku', $variantSkus)->exists()) {
            $validator->errors()->add('variants', 'Uno de los SKU de variante ya existe en otro producto.');
        }

        if ($variantBarcodes->isNotEmpty() && ProductVariant::query()->whereIn('barcode', $variantBarcodes)->exists()) {
